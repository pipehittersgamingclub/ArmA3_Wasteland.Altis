	...
	switch (true) do
	{
		...
		case (!isNull _displayDebug): //Debug panel
		{
			...
			switch (lbCurSel _debugSelect) do
			{
				...
				{
					_confirmMsg = format ["This will unlock all base objects within 5m<br/>"];
					_confirmMsg = _confirmMsg + format ["<br/>Unlock Objects (5m)? "];

					if ([parseText _confirmMsg, "Confirm", "CONFIRM", true] call BIS_fnc_guiMessage) then
					{
					{_x setVariable ["objectLocked",false,true];} forEach (position player nearObjects ["All",5]);
					hint format["You have unlocked all base objects within 5m of the area"];
					};
				 };
				case 8: //Delete Unlocked Base Objects within 50m - Requested by PatPgtips
				{
					_confirmMsg = format ["This will delete all unlocked base objects within 5m<br/>"];
					_confirmMsg = _confirmMsg + format ["<br/>Delete Objects?"];

					if ([parseText _confirmMsg, "Confirm", "CONFIRM", true] call BIS_fnc_guiMessage) then
					{
					
					{if !(_x getVariable ["objectLocked",false]) then {deleteVehicle _x};} forEach (position player nearObjects ["All",5]);
					hint format["You have deleted all unlocked base objects within 5m of the area"];
					};
